 ...
 * @ingroup conversions
 *
 * @brief An information about allownes of invalid Data instance conversion.
 *
 * @details By default, no conversions allowed for invalid Data instances.
 */
template<typename>
struct Is_invalid_data_conversion_allowed final : std::false_type {};
 ...
 *
 * @details Conversions to std::optional<T> are allowed for invalid Data instances.
 */
template<typename T>
struct Is_invalid_data_conversion_allowed<std::optional<T>> final : std::true_type {};
 ...
 *
 * @par Requires
 * `Is_invalid_data_conversion_allowed<T>::value || data`.
 *
...
{
  if constexpr (!Is_invalid_data_conversion_allowed<T>::value) {
    if (!data)
      throw Generic_exception{"cannot convert to type: invalid Data instance"};
  }
 ...
 *
 * @par Requires
 * `Is_invalid_data_conversion_allowed<T>::value || data && *data`.
 */
template<typename T, typename ... Types>
inline T to(std::unique_ptr<Data>&& data, Types&& ... args)
{
  if constexpr (!Is_invalid_data_conversion_allowed<T>::value) {
    if (!data || !(*data))
      throw Generic_exception{"cannot convert to type: invalid data"};
  }
