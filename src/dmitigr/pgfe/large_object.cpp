...
{
  if (is_valid())
    throw Generic_exception{"cannot assign large object"};
...
{
  if (!is_valid())
    throw Generic_exception{"cannot seek large object"};
...
{
  if (!is_valid())
    throw Generic_exception{"cannot tell large object"};
...
DMITIGR_PGFE_INLINE void Large_object::truncate(const std::int_fast64_t new_size)
{
  if (!(is_valid() && new_size >= 0))
    throw Generic_exception{"cannot truncate large object"};
...
Large_object::read(char* const buf, const std::size_t size)
{
  if (!(is_valid() && buf && size <= std::numeric_limits<int>::max()))
    throw Generic_exception{"cannot read large object"};
...
{
  if (!(is_valid() && buf && size <= std::numeric_limits<int>::max()))
    throw Generic_exception{"cannot write large object"};
...
{
  if (!is_valid())
    throw Generic_exception{"cannot get connection of invalid large object"};
